...
- [docker-environment-spec.md](/home/deepblue/AV_Drone/docs/docker-environment-spec.md)
- [command-reference.md](/home/deepblue/AV_Drone/docs/command-reference.md)
- [multi-drone-development-roadmap.md](/home/deepblue/AV_Drone/docs/multi-drone-development-roadmap.md)
- [portfolio-system-engineering-guide.md](/home/deepblue/AV_Drone/docs/portfolio-system-engineering-guide.md)
...
- `Safety`: timeout, stale command, emergency stop 감시
- `Metrics`: 실행 결과와 실험 지표 수집

한 줄로 요약하면 다음과 같다.

`Gazebo` 안에서 드론을 띄우고, `PX4 SITL`이 비행 제어기를 흉내 내며, `MAVROS`가 PX4와 ROS 2를 연결하고, 이 저장소의 `planner/control/safety/metrics` 파이프라인이 자율주행과 검증을 수행하는 구조다.
...
│ │     ├─ drone_bringup / control / perception / planning       │
│ │     └─ drone_safety / drone_metrics                          │
│ └─ X11                                                         │
...
Autonomy Pipeline
  - perception: /drone1/scan -> nearest obstacle
  - planner: obstacle distance -> autonomy cmd
  - safety: timeout / obstacle threshold -> safe cmd
  - control: safe cmd -> /mavros/setpoint_velocity/cmd_vel
  - metrics: artifact logging and run summary
```
...
- [`src/drone_control`](/home/deepblue/AV_Drone/src/drone_control)
- [`src/drone_perception`](/home/deepblue/AV_Drone/src/drone_perception)
- [`src/drone_planning`](/home/deepblue/AV_Drone/src/drone_planning)
- [`src/drone_safety`](/home/deepblue/AV_Drone/src/drone_safety)
- [`src/drone_metrics`](/home/deepblue/AV_Drone/src/drone_metrics)
...
- 기존 검증 완료 흐름: `mppi.launch.py` 기반 단일 드론 MPPI 데모
- 확장 중인 흐름: `single_drone_autonomy.launch.py` 기반 센서 자율주행 구조

현재까지 확인된 점:

- `mppi.launch.py`는 PX4 SITL, Gazebo, MAVROS, MPPI 상태 머신까지 정상 동작했다.
- `single_drone_autonomy.launch.py`는 perception, planning, safety, control, metrics 뼈대와 artifact 기록 구조까지 올라온 상태다.
- 라이다 문제를 해결하기 위해 `sim` 컨테이너를 `Ubuntu 20.04(focal)` 기반 PX4 이미지에서 `Ubuntu 22.04 + ROS 2 Humble` 기준으로 옮기는 작업을 진행 중이다.

즉, 지금의 핵심 작업은 "센서가 실제로 ROS 2 `LaserScan` 토픽으로 보이게 만드는 것"이다.

## 다음 작업

현재 기준 다음 우선순위는 아래 순서가 맞다.

1. `sim` 이미지 빌드를 끝내고 `/drone1/scan` publisher를 확인한다.
2. 라이다 입력이 실제로 들어오면 perception/safety 파이프라인이 정상 반응하는지 확인한다.
3. 그 다음 `single_drone_autonomy`에 takeoff/offboard mission manager를 붙여 실제 비행 launch로 만든다.
4. 이후 단순 obstacle world를 추가해 센서 기반 회피를 검증한다.
5. 마지막으로 메트릭, fail-safe, 자동 실험 러너를 더 강화한다.
...
│  │  ├─ config/
│  │  │  └─ drone1_autonomy.yaml
│  │  └─ package.xml
│  ├─ drone_control/
│  │  ├─ drone_control/
│  │  │  ├─ autonomy_manager_node.py
...
│  ├─ architecture.md
│  ├─ multi-drone-development-roadmap.md
│  └─ portfolio-system-engineering-guide.md
├─ docker-compose.yml
└─ README.md
```
...
이 프로젝트는 현재 두 층으로 이해하면 된다.

1. 기존 단일 드론 MPPI 데모
2. 새 단일 드론 센서 자율주행 뼈대
...
새로 추가된 패키지는 센서 기반 단일 드론 자율주행으로 확장하기 위한 기초공사다.

- [`src/drone_bringup`](/home/deepblue/AV_Drone/src/drone_bringup): launch와 YAML 파라미터
- [`src/drone_control`](/home/deepblue/AV_Drone/src/drone_control): MAVROS 오프보드 제어 인터페이스
- [`src/drone_perception`](/home/deepblue/AV_Drone/src/drone_perception): LiDAR 기반 장애물 거리 전처리
- [`src/drone_planning`](/home/deepblue/AV_Drone/src/drone_planning): 센서 기반 planner command 생성
- [`src/drone_safety`](/home/deepblue/AV_Drone/src/drone_safety): timeout / emergency stop fail-safe
- [`src/drone_metrics`](/home/deepblue/AV_Drone/src/drone_metrics): runtime metrics와 artifact 저장

현재 새 파이프라인의 흐름은 다음과 같다.
...
- 멀티드론 로드맵: [docs/multi-drone-development-roadmap.md](/home/deepblue/AV_Drone/docs/multi-drone-development-roadmap.md)
- 포트폴리오 시스템 엔지니어링 가이드: [docs/portfolio-system-engineering-guide.md](/home/deepblue/AV_Drone/docs/portfolio-system-engineering-guide.md)
